...
function rebuildNav(m, key) {
  ...
  return buildNav({ walls: m.walls, props: m.props, w: m.w, d: m.d, seeds });
}
  ...
  updateSpectators(dt) {
    ...
    for (const p of this.players.values()) {
      ...
      if (inp.f || inp.s) {
        ...
        p.x = clamp(p.x + (dx / len) * sp, -this.map.w / 2 + 1, this.map.w / 2 - 1);
        p.z = clamp(p.z + (dz / len) * sp, -this.map.d / 2 + 1, this.map.d / 2 - 1);
      }
    ...
    // กลับเข้ามาหลังเน็ตหลุด: คืนบทบาท ร่าง ตำแหน่งเดิม
    {
      ...
      if (held && held.room && rooms.get(held.room.id) === held.room && held.room.players.get(held.player.id)) {
        ...
        socket.emit('init', {
          ...
          cfg: { hide: CFG.HIDE_TIME, hunt: CFG.HUNT_TIME, catchRange: CFG.CATCH_RANGE, revive: CFG.REVIVE_COST, rerollCost: CFG.REROLL_COST, leavePenalty: CFG.LEAVE_PENALTY, runMax: CFG.RUN_MAX,
            specSpeed: CFG.SPEC_SPEED, specHeight: CFG.SPEC_HEIGHT },
        });
