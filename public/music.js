...
/* ปลดล็อกเสียงจากการแตะจอ: iOS/Chrome ให้เปิดเสียงได้เฉพาะในจังหวะที่ผู้ใช้แตะเท่านั้น
   และเวลาสลับแอปกลับมา ระบบจะพักเสียงไว้ ต้องปลุกใหม่ด้วยการแตะครั้งถัดไป */
export function unlock(wantOn) {
  try {
    if (ctx && ctx.state !== 'running') ctx.resume();
    if (wantOn && !playing) start();
  } catch (e) {}
}
