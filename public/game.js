...
const el = {
  ...
  chatSend: $('chatSend'),
  spect: $('spect'),
  rollCost: $('rollCost'), rollSub: $('rollSub'), btnQuit: $('btnQuit'), roomTag: $('roomTag'),
  revive: $('revive'), reviveCost: $('reviveCost'), reviveBar: $('reviveBar'), reconnect: $('reconnect'),
...
scene.fog = null;      // ไม่ใช้หมอกแล้ว ของประจำฉากส่งครบตั้งแต่โหลดแมป จึงไม่มีของโผล่ให้ต้องบัง
...
let SPEC_SPEED = 18, SPEC_HEIGHT = 4.5;
const CAM_OUT = 15;       // กล้องส่อง (ล็อกท่า/ผู้ชม) เลื่อนเลยขอบแมปออกไปได้กี่เมตร
...
/* ช่วงก้ม-เงยของกล้อง: -0.6 rad (ก้มมองพื้นจากด้านหน้า) ถึง 1.25 rad (มองจากมุมสูงเกือบตั้งฉาก) */
const PITCH_MIN = -0.6, PITCH_MAX = 1.25;
const move = { f: 0, s: 0 };
...
let mapGroup = null;
let roofMesh = null;
/* ปรับสีให้เข้ม/สว่างขึ้น ใช้ทำขอบหลังคา */
...
function buildMap(m) {
  ...
  built = true;
  mapKeyBuilt = key;
  const theme = m.theme || (m.map && m.map.theme) || 'mall';
  const zoo = theme === 'zoo';
  ...
  if (school) {
    // โรงเรียน: สนามหญ้ารอบ อาคารพื้นกระเบื้อง สนามบาสพื้นยาง ลานเสาธงพื้นปูน
    const grass = new THREE.Mesh(new THREE.PlaneGeometry(m.w, m.d), new THREE.MeshLambertMaterial({ color: 0x86c96a }));
    grass.rotation.x = -Math.PI / 2;
    mapGroup.add(grass);
    mapGroup.add(new THREE.Mesh(mergeBoxes([
      { w: 74, h: 0.04, d: 34, c: 0xe6e2d8, x: 0, y: 0.012, z: 8 },     // อาคารเรียนแถวบน + โถง
      { w: 74, h: 0.04, d: 14, c: 0xe6e2d8, x: 0, y: 0.012, z: -18 },   // อาคารแถวล่าง
      { w: 30, h: 0.03, d: 6, c: 0xcfcabf, x: -5, y: 0.014, z: -9 },    // ลานเสาธง
      { w: 12, h: 0.03, d: 18, c: 0xd8a66a, x: 32, y: 0.014, z: -4 },   // สนามบาส
    ]), vertexColorMat));
  } else if (house) {
    // บ้าน: สนามหญ้ารอบบ้าน พื้นบ้านไม้ ทางเดินปูนหน้าบ้าน
    const grass = new THREE.Mesh(new THREE.PlaneGeometry(m.w, m.d), new THREE.MeshLambertMaterial({ color: 0x86c96a }));
    grass.rotation.x = -Math.PI / 2;
    mapGroup.add(grass);
    mapGroup.add(new THREE.Mesh(mergeBoxes([
      { w: 52, h: 0.04, d: 40, c: 0xd8b98a, x: -10, y: 0.012, z: -2 },    // พื้นไม้ทั้งหลัง
      { w: 6, h: 0.03, d: 10, c: 0xcfcac0, x: -39, y: 0.014, z: 0 },      // ทางเดินหน้าประตู
      { w: 8, h: 0.03, d: 6, c: 0xb8bcc2, x: 20, y: 0.014, z: 10 },       // ลานหน้าประตูโรงรถ
      { w: 6, h: 0.03, d: 8, c: 0xcfcac0, x: 0, y: 0.014, z: 24 },        // ทางเข้ารั้ว
    ]), vertexColorMat));
  } else if (zoo) {
  ...
  const wallParts = [{ w: m.w + 1.6, h: 0.5, d: m.d + 1.6, c: (zoo || house || school) ? 0x6f9a55 : 0xcfc7b6, x: 0, y: -0.26, z: 0 }];
  for (const w of m.walls) {
    ...
    const wallCol = school ? (outer ? 0xf3ead6 : 0xeadfc8)
      : house ? (outer ? 0xf6efe0 : 0xf1e6d2) : outer ? (zoo ? 0xd8c9a8 : 0xf3ede1) : 0xe4dccc;
    const capCol = school ? 0x3f7fbf : house ? 0xb3543f : outer ? (zoo ? 0x5f8f4a : 0x6aa8d8) : 0xb7ab95;
    wallParts.push({ w: w.w, h: w.h, d: w.d, c: wallCol, x: cx, y: w.h / 2, z: cz });
    wallParts.push({ w: w.w + 0.16, h: 0.22, d: w.d + 0.16, c: capCol, x: cx, y: w.h + 0.05, z: cz });
  ...
  // ป้ายชื่อโซน พื้นแยกสี กระจกหน้าร้าน — ใช้ข้อมูลโซนที่เซิร์ฟเวอร์ส่งมา
  {
    ...
    const floorCols = zoo
      ...
      : { shop: null };
    const parts = [], floors = [], roofs = [], roofRooms = [];
    zones.forEach((zn, i) => {
      const [x0, x1, z0, z1] = zn.box;
      const cx = (x0 + x1) / 2, cz = (z0 + z1) / 2;
      const w = x1 - x0, d = z1 - z0;
      // พื้นโซน
      const fc = zn.kind === 'yard' ? null : zn.color ? parseInt(zn.color.replace('#', ''), 16)
      ...
      /* หลังคาอาคาร: ห้องที่มีผนังล้อมรอบจะมีหลังคาคลุมด้วย
         วางไว้เหนือหัว 3.6 ม. มองจากมุมกล้องเฉียงยังเห็นในห้องได้ปกติ
         (ไม่ใส่ให้คอกสัตว์กับสนาม เพราะเป็นพื้นที่กลางแจ้ง) */
      if (zn.door && zn.kind !== 'pen' && zn.kind !== 'water' && zn.kind !== 'yard') {
        const rc = school ? 0xb3543f : house ? 0xb3543f : zoo ? 0x6f8f5a : 0xc07a5a;
        roofRooms.push([x0, x1, z0, z1]);
        roofs.push({ w: w + 0.5, h: 0.24, d: d + 0.5, c: rc, x: cx, y: 3.72, z: cz });
        roofs.push({ w: w + 0.9, h: 0.16, d: 0.5, c: shadeHex(rc, -0.12), x: cx, y: 3.62, z: z0 });
        roofs.push({ w: w + 0.9, h: 0.16, d: 0.5, c: shadeHex(rc, -0.12), x: cx, y: 3.62, z: z1 });
        roofs.push({ w: 0.5, h: 0.16, d: d + 0.9, c: shadeHex(rc, -0.12), x: x0, y: 3.62, z: cz });
        roofs.push({ w: 0.5, h: 0.16, d: d + 0.9, c: shadeHex(rc, -0.12), x: x1, y: 3.62, z: cz });
        // สันหลังคาเตี้ย ๆ ให้ดูเป็นทรงจั่วจาง ๆ
        roofs.push({ w: Math.max(1, w - 2), h: 0.18, d: Math.max(1, d - 2), c: shadeHex(rc, 0.1), x: cx, y: 3.92, z: cz });
      }
      // ป้ายชื่อเหนือประตู (เฉพาะร้าน/อาคาร) ห้องในบ้านกับสนามไม่มีป้าย
      if (zn.kind === 'pen' || zn.kind === 'water' || zn.kind === 'room' || zn.kind === 'yard') return;
    ...
    if (roofs.length) {
      // หลังคาอยู่กลุ่มแยก จะได้ซ่อนได้เวลากล้องอยู่ในห้องนั้น
      roofMesh = new THREE.Mesh(mergeBoxes(roofs), vertexColorMat);
      roofMesh.userData.rooms = roofRooms;
      mapGroup.add(roofMesh);
    }
    if (!zoo) {
      // ลานกลางห้าง
...
/* ของประจำฉากอยู่ถาวร ไม่ถูกลบตอนรับแพ็กเก็ต (สตรีมมาเฉพาะคนซ่อน) */
function seedStatic(props) {
  for (const [k, o] of objs) if (o.st) objs.delete(k);
  for (const p of props) {
    if (p.id === undefined) continue;
    objs.set(p.id, { i: p.id, st: true, x: p.x, z: p.z, rx: p.x, rz: p.z,
      ry2: p.y || 0, h: p.y || 0, t: p.t, r: p.r || 0, m: 0, dirty: true });
  }
  slotDirty = true;
}
...
function fillLobbyProps() {
  ...
  for (const p of LOBBY_PROPS) {
    objs.set(id, { i: id, st: true, x: p.x, z: p.z, rx: p.x, rz: p.z,
      ry2: p.y || 0, h: p.y || 0, t: p.t, r: p.r, m: 0, dirty: true });
    id++;
  }
}
...
export function startGame(opts) {
  ...
  // ทั้งครั้งแรกและตอนต่อกลับมาหลังเน็ตสะดุด ส่ง join ห้องเดิม → เซิร์ฟเวอร์คืนที่นั่งให้
  let joinedRoom = opts.room || null;
  socket.on('connect', () => {
    socket.emit('join', { token: opts.token, room: joinedRoom });
  ...
  /* เน็ตสะดุดไม่เด้งออกล็อบบี้ทันที: โชว์ป้ายกำลังเชื่อมต่อใหม่ แล้วให้ socket.io ต่อกลับเอง
     ถ้าต่อไม่ได้ภายใน 60 วินาที (เท่ากับที่เซิร์ฟเวอร์เก็บที่นั่งไว้) ค่อยกลับล็อบบี้ */
  socket.on('disconnect', (reason) => {
    if (!joined) return;
    if (reason === 'io server disconnect') { quit(); return; }   // เซิร์ฟเวอร์สั่งปิดเอง (เช่นถูกเตะ)
    el.reconnect.classList.remove('hidden');
    if (reconnectTimer) clearTimeout(reconnectTimer);
    reconnectTimer = setTimeout(() => {
      reconnectTimer = null;
      if (!socket || !socket.connected) { toast('เชื่อมต่อใหม่ไม่สำเร็จ'); quit(); }
    }, 60000);
  });

  socket.on('init', (m) => {
    if (m.room) joinedRoom = m.room;
    ...
    if (m.specSpeed) SPEC_SPEED = m.specSpeed;
    if (m.specHeight) SPEC_HEIGHT = m.specHeight;
    if (m.cfg && m.cfg.specSpeed) SPEC_SPEED = m.cfg.specSpeed;
    if (m.cfg && m.cfg.specHeight) SPEC_HEIGHT = m.cfg.specHeight;
    if (m.jumpV) JUMPV = m.jumpV;
    if (m.gravity) GRAV = m.gravity;
    buildMap(m);
    music.setTune(m.key || 'lobby');            // เพลงเปลี่ยนตามด่าน
    // ล้างก่อน แล้วค่อยวางของประจำฉาก (ลำดับสลับกันจะทำให้ของหายทั้งแมป)
    objs.clear();
    seedStatic(m.props || []);
    slotDirty = true;
    pending.length = 0;
  ...
  socket.on('m', onMe);
  socket.on('map', (m) => {
    MAP = Object.assign({}, MAP || {}, { w: m.w, d: m.d, walls: m.walls, key: m.key,
      theme: m.theme, name: m.name, zones: m.zones || [], props: m.props || [] });
    buildSolids(m.props || []);
    buildMap(MAP);
    music.setTune(m.key || 'lobby');
    objs.clear();
    seedStatic(m.props || []);
  });
}
...
export function quit() {
  if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; }
  if (el.reconnect) el.reconnect.classList.add('hidden');
  if (socket) {
  ...
  music.setTune('lobby');                        // กลับล็อบบี้ใช้เพลงล็อบบี้
  fillLobbyProps();
...
function decodeObjects(buf) {
  ...
  for (const [k, o] of objs) if (!seen.has(k) && !o.st) { objs.delete(k); slotDirty = true; }
}
...
function handleFx(f) {
  if (f.k === 'catch') {
    ...
    pushFeed(`<em>${esc(f.n)} เกิดใหม่แล้ว</em>`);
    ...
    pushFeed(`<em>${esc(f.n)} หลุดการเชื่อมต่อ (รอกลับ 60 วิ)</em>`);
  } else if (f.k === 'back') {
    pushFeed(`<em>${esc(f.n)} กลับมาแล้ว</em>`);
  } else if (f.k === 'phase') {
...
function syncMates(dt) {
  ...
  for (const m of list) {
    ...
    if (!tag || tag.name !== m.n || tag.seek !== !!m.s) {
      if (tag) scene.remove(tag.sprite);
      const sprite = makeLabel(m.n, m.s ? '#ffb4a2' : '#9fe6b8');
      sprite.scale.set(2.4, 0.6, 1);
      scene.add(sprite);
      tag = { sprite, name: m.n, seek: !!m.s };
...
function paintHud() {
  ...
  setText(el.roleHint, out
    ? 'นั่งดูรอบนี้ต่อ หรือจ่ายทองเกิดใหม่ถ้ายังทัน'
    : seeker
  ...
  // เปลี่ยนร่าง: ครั้งแรกฟรี หลังจากนั้นโชว์เวลาที่ต้องรอ (ไม่ใช้ทอง)
  setText(el.rollSub, ME.rrCd > 0 ? `รอ ${ME.rrCd} วิ` : 'ฟรี');
  el.btnRoll.classList.toggle('hidden', !hiding);
  el.btnRoll.disabled = ME.rrCd > 0 || ME.pool < 2;
  ...
  if (ME.rev > 0) {
    ...
    if (el.reviveBar) {
      const pct = Math.max(0, Math.min(1, ME.revLeft / (ME.revWin || 15)));
      el.reviveBar.style.width = (pct * 100).toFixed(1) + '%';
      el.reviveBar.classList.toggle('low', pct < 0.34);
    }
    el.btnRevive.disabled = (ME.gold || 0) < ME.rev;
...
function paintRoster() {
  ...
  for (const p of list) {
    ...
    // ปุ่มแอดเพื่อนโชว์เฉพาะคนจริงที่ยังไม่ได้เป็นเพื่อนกัน
    let act = `<s>${TH[p.r] || p.r}</s>`;
    if (!p.b && p.n !== meName) {
      const st = fr[p.n];
      if (st === 'f') act = '<s class="fr ok">เพื่อนแล้ว</s>';
      else if (st === 's') act = '<s class="fr wait">ส่งคำขอแล้ว</s>';
      else if (st === 'r') act = `<button class="fr add" data-add="${esc(p.n)}">ตอบรับ</button>`;
      else act = `<button class="fr add" data-add="${esc(p.n)}">+ เพื่อน</button>`;
    }
...
/* กดปุ่มแอดเพื่อนในรายชื่อระหว่างเล่น */
document.addEventListener('click', (e) => {
  const b = e.target.closest && e.target.closest('[data-add]');
  if (!b || !el.roster.contains(b)) return;
  const name = b.getAttribute('data-add');
  if (socket) socket.emit('friendAdd', name);
...
addEventListener('keydown', (e) => {
  ...
  else if ((e.code === 'ShiftLeft' || e.code === 'ShiftRight') && !e.repeat) act(ME && ME.run ? 'runOff' : 'run');
...
addEventListener('keyup', (e) => {
  keys[e.code] = 0;
  // Shift ใช้แบบกดสลับแล้ว ปล่อยแล้วไม่ต้องหยุดวิ่ง
...
function bindPointer() {
  ...
  addEventListener('mousemove', (e) => {
    ...
    yaw -= dx * 0.006;
    pitch = Math.max(PITCH_MIN, Math.min(PITCH_MAX, pitch - dy * 0.0062));
    dragPrev = { x: e.clientX, y: e.clientY };
  });
  ...
  addEventListener('touchmove', (e) => {
    ...
    for (const t of e.changedTouches) {
      ...
      if (looks.size === 1) {
        ...
        pitch = Math.max(PITCH_MIN, Math.min(PITCH_MAX, pitch - (t.clientY - prev.y) * 0.0062));
      }
...
{
  // แตะครั้งเดียวเริ่มวิ่ง แตะอีกครั้งหยุด ไม่ต้องกดค้าง (กดค้างบนมือถือลำบาก)
  // แตะครั้งเดียวเริ่มวิ่ง แตะอีกครั้งหยุด (กดค้างบนมือถือลำบาก)
  // เซิร์ฟเวอร์จะดับให้เองเมื่อพลังหมด แล้วแตะใหม่ได้เลยตอนพลังกลับมา
  const toggleRun = (e) => {
    if (e && e.cancelable) e.preventDefault();
    act(ME && ME.run ? 'runOff' : 'run');
  };
  el.btnRun.addEventListener('touchstart', toggleRun, { passive: false });
  el.btnRun.addEventListener('mousedown', toggleRun);
}
...
function simulate(c) {
  if (ME && (ME.role === 'out' || ME.role === 'wait')) {
    // ผู้ชม: ทำนายการเลื่อนกล้องเองด้วยสูตรเดียวกับเซิร์ฟเวอร์
    // (ถ้ารอตำแหน่งจากเซิร์ฟเวอร์อย่างเดียว กล้องจะกระตุกตามอัตราแพ็กเก็ตทั้งที่ FPS เต็ม)
    const sp = SPEC_SPEED * FIXED;
    if (c.f || c.s) {
      const dx = Math.sin(c.yaw) * c.f - Math.cos(c.yaw) * c.s;
      const dz = Math.cos(c.yaw) * c.f + Math.sin(c.yaw) * c.s;
      const len = Math.hypot(dx, dz) || 1;
      const mw = (MAP && MAP.w) || 80, md = (MAP && MAP.d) || 60;
      local.x = Math.max(-mw / 2 - CAM_OUT, Math.min(mw / 2 + CAM_OUT, local.x + (dx / len) * sp));
      local.z = Math.max(-md / 2 - CAM_OUT, Math.min(md / 2 + CAM_OUT, local.z + (dz / len) * sp));
    }
    local.y = SPEC_HEIGHT; local.vy = 0; local.ground = true;
    return;
  }
  if (ME && ME.hold) {
...
function stepLocal(dt) {
  ...
  // ล็อกท่า = เลื่อนกล้องรอบตัวเอง (ฝั่งไคลเอนต์)
  // เป็นผู้ชม = เซิร์ฟเวอร์ขยับตัวเราให้บินดูทั้งแมป จึงส่งอินพุตจริงไปเหมือนตอนเล่น
  const wantFree = !!(ME && ME.lock && ME.role !== 'out' && ME.role !== 'wait');
  ...
  if (freeMode) {
    ...
    const sp = SPEC_SPEED * dt;
    freeCam.x += (Math.sin(yaw) * f - Math.cos(yaw) * sd) * sp;
    freeCam.z += (Math.cos(yaw) * f + Math.sin(yaw) * sd) * sp;
    // เลื่อนไปดูได้ทั้งแมป และเลยขอบออกไปได้อีกหน่อย (เดิมจำกัดแค่ 34 ม. จากตัว เลยเหมือนชนกำแพงล่องหน)
    if (MAP && MAP.w) {
      const ox = MAP.w / 2 + CAM_OUT, oz = MAP.d / 2 + CAM_OUT;
      freeCam.x = Math.max(-ox - local.x, Math.min(ox - local.x, freeCam.x));
      freeCam.z = Math.max(-oz - local.z, Math.min(oz - local.z, freeCam.z));
    }
...
function updateCamera(dt) {
  ...
  // เลื่อนกล้องส่องออกไปไกลจากตัว ยกมุมมองขึ้นสูงเท่ากล้องผู้ชม จะได้มองข้ามผนังเห็นเพื่อน
  const peek = freeMode && Math.hypot(freeCam.x, freeCam.z) > 2;
  const baseY = peek ? Math.max(viewPos.y + h * 0.5 + 0.55, SPEC_HEIGHT) : viewPos.y + h * 0.5 + 0.55;
  camTarget.set(viewPos.x + freeCam.x, baseY, viewPos.z + freeCam.z);
  const cp = Math.cos(pitch), sp = Math.sin(pitch);
  camBack.set(Math.sin(yaw) * cp, sp, Math.cos(yaw) * cp);
  const back = camBack;
  let dist = camDist;
  // กล้องที่หลุดจากตัว (ผู้ชม หรือล็อกท่าแล้วเลื่อนกล้องส่อง) ทะลุผนังได้ ไม่ถูกกระชากเข้ามา
  const ghostCam = !!(ME && (ME.role === 'out' || ME.role === 'wait')) || freeMode;
  if (!ghostCam) for (let i = 10; i >= 1; i--) {
    const t = (i / 10) * dist;
  ...
  // กล้องผู้ชมทะลุผนังได้ แต่ต้องไม่หลุดออกนอกขอบแมป (เดิมเลื่อนเลยรั้วออกไปได้)
  if (ghostCam && MAP && MAP.w) {
    const lx = MAP.w / 2 + CAM_OUT, lz = MAP.d / 2 + CAM_OUT;
    camWant.x = Math.max(-lx, Math.min(lx, camWant.x));
    camWant.z = Math.max(-lz, Math.min(lz, camWant.z));
  }
  camera.position.lerp(camWant, Math.min(1, dt * 12));

  /* อยู่ในห้องไหน ซ่อนหลังคาห้องนั้น ไม่งั้นมองไม่เห็นตัวเอง
     (ซ่อนทั้งก้อนเพราะหลังคาทุกห้องรวมเป็นก้อนเดียวเพื่อประหยัด draw call
      ห้องมีผนังกั้นอยู่แล้ว จึงไม่ทำให้เห็นห้องอื่นทะลุ) */
  if (roofMesh) {
    const rooms = roofMesh.userData.rooms || [];
    const inside = rooms.some((b) => viewPos.x > b[0] - 0.5 && viewPos.x < b[1] + 0.5
      && viewPos.z > b[2] - 0.5 && viewPos.z < b[3] + 0.5);
    roofMesh.visible = !inside;
  }
...
function frame(now) {
  ...
  // ผู้ชม: จุดที่กล้องมองต้องอยู่ในแมปเสมอ (กันค่าคลาดเคลื่อนสะสมพาหลุดขอบ)
  if (ME && (ME.role === 'out' || ME.role === 'wait') && MAP && MAP.w) {
    const mx = MAP.w / 2 + CAM_OUT, mz = MAP.d / 2 + CAM_OUT;
    viewPos.x = Math.max(-mx, Math.min(mx, viewPos.x));
    viewPos.z = Math.max(-mz, Math.min(mz, viewPos.z));
  }
  if (myProp) {
