...
function buildMap(m) {
  ...
  if (school) {
    ...
    mapGroup.add(new THREE.Mesh(mergeBoxes([
      ...
      { w: 8, h: 0.03, d: 10, c: 0xb8bcc2, x: 20, y: 0.014, z: 9.3 },     // ลานโรงรถ
...
function simulate(c) {
  if (ME && (ME.role === 'out' || ME.role === 'wait')) {
    ...
    if (c.f || c.s) {
      ...
      local.x = Math.max(-MAP.w / 2 + 1, Math.min(MAP.w / 2 - 1, local.x + (dx / len) * sp));
      local.z = Math.max(-MAP.d / 2 + 1, Math.min(MAP.d / 2 - 1, local.z + (dz / len) * sp));
    }
...
function updateCamera(dt) {
  ...
  const back = camBack;
  let dist = camDist;
  const ghostCam = !!(ME && (ME.role === 'out' || ME.role === 'wait'));
