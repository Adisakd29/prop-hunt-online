...
const I18N = {
  th: {
    join: 'เล่นเลย', roomNo: 'ห้อง {n}', roomEmpty: 'ห้องนี้ยังไม่มีใคร กดเข้าร่วมเกมเพื่อเปิดห้อง',
    ...
    tabSwitch: 'เปลี่ยนห้อง', tabRoom: 'ห้อง', tabFriends: 'เพื่อน', tabOpt: 'ตัวเลือก',
    language: 'ภาษา', avatar: 'รูปประจำตัว', friends: 'เพื่อน', logout: 'ออกจากระบบ',
    picked: 'เลือก {r} แล้ว กดเข้าร่วมเกมได้เลย',
  },
  en: {
    join: 'PLAY', roomNo: 'Room {n}', roomEmpty: 'Nobody here yet — press join to open it',
    ...
    tabSwitch: 'Switch room', tabRoom: 'Rooms', tabFriends: 'Friends', tabOpt: 'Options',
...
const PANELS = ['pHide', 'pSeek', 'pShop', 'pRooms', 'pFriends', 'pOptions'];
let curPanel = null;
...
function showPanel(id) {
  ...
  if (curPanel === 'pOptions') loadAvatars();
  if (curPanel === 'pFriends') loadFriends();
}
...
function paintTopRoom() {
  ...
  // สถานะใต้ปุ่มเล่น: ห้อง · คน/สูงสุด · กำลังทำอะไร
  {
    const n2 = show ? (show.players + show.bots) : null;
    const mx = show ? show.max : 20;
    const st = !show ? 'ห้องว่าง เข้าได้เลย'
      : show.phase === 'lobby' ? 'กำลังรอผู้เล่น'
      : show.phase === 'hide' ? 'กำลังซ่อนตัว เข้าทันได้'
      : show.phase === 'hunt' ? `กำลังล่ากัน เหลือ ${fmt(show.tl)}`
      : 'จบรอบ กำลังเริ่มใหม่';
    const busy = show && show.phase === 'hunt';
    $('playStatus').innerHTML = `${t('roomNo', { n: no })} · ${n2 === null ? '—' : n2} / ${mx} · <em class="${busy ? 'busy' : ''}">${st}</em>`;
  }

  // รายชื่อผู้เล่นในห้องที่เลือกอยู่
  $('rpTitle').textContent = t('roomNo', { n: no });
  $('rpCount').textContent = show ? `${show.players + show.bots}/${show.max}` : `— / 20`;
...
function paintShop() {
  ...
  $('hideCount').textContent = items.length ? `${items.filter((i) => i.owned).length} / ${items.length}` : '— / —';
  // ของแยกตามด่าน ไม่ปนกัน
  const groups = [['school', 'propGrid', 'hideCountMall'], ['zoo', 'propGridZoo', 'hideCountZoo'], ['house', 'propGridHouse', 'hideCountHouse']];
  for (const [mp, grid, cnt] of groups) {
    const list = items.filter((i) => i.map === mp);
    $(cnt).textContent = `${list.filter((i) => i.owned).length} / ${list.length}`;
    $(grid).innerHTML = '';
    list.forEach((it, i) => $(grid).appendChild(card(it, it.id, i + 1)));
  }
...
$('gachaProp').onclick = () => gacha('prop', 'school');
$('gachaZoo').onclick = () => gacha('prop', 'zoo');
$('gachaHouse').onclick = () => gacha('prop', 'house');
...
async function loadFriends() {
  ...
  try {
    ...
    if ($('frCount')) $('frCount').textContent = `${(r.friends || []).filter((f) => f.online).length} ออนไลน์ / ${(r.friends || []).length} คน`;
...
function play(room) {
  ...
  startGame({
    ...
    onEnter: () => { if (musicWanted()) { music.unlock(true); music.setVolume(0.4); paintMusic(); } },
...
$('btnPlay').onclick = () => {
  // เปิดเพลงในจังหวะที่กดปุ่ม (ถ้ารอไปเปิดตอนโหลดด่านเสร็จ เบราว์เซอร์จะไม่ยอมให้มีเสียง)
  if (musicWanted()) { music.unlock(true); music.setVolume(0.4); paintMusic(); }
  play(pickedRoom);
};
...
/* เปิดเพลงอัตโนมัติ: เบราว์เซอร์ให้เปิดเสียงได้เฉพาะตอนผู้ใช้แตะจอ
   จึงฟังทุกการแตะ (ไม่ใช่แค่ครั้งแรก) เผื่อระบบพักเสียงไว้ตอนสลับแอปแล้วกลับมา
   iOS ปลดล็อกเสียงได้จาก touchend/click เท่านั้น pointerdown อย่างเดียวไม่พอ */
...
function autoMusic() {
  ...
  music.unlock(want);
  if (want) music.setVolume(0.45);
  paintMusic();
}
for (const ev of ['pointerdown', 'touchend', 'click', 'keydown']) {
  addEventListener(ev, autoMusic, { passive: true, capture: true });
}
// กลับมาที่แท็บ/แอป ลองปลุกเสียงทันที (บางเครื่องยอมโดยไม่ต้องแตะ)
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible' && musicWanted()) music.unlock(true);
});
...
(function loadIconSheets() {
  const sheets = [['ui/icons.png', 'has3d'], ['ui/icons2.png', 'has2'],
    ['ui/timer.png', 'hasTimer'], ['ui/lock.png', 'hasLock'], ['ui/run.png', 'hasRun'],
    ['ui/friends.png', 'hasFriends']];
