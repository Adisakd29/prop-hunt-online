...
const ROOMS = [
  ...
  { id: 'hall', name: 'โถงกลาง', box: [-35, 35, -8, 4], door: null, color: '#e6e2d8', kind: 'open' },
  ...
  { id: 'toilet', name: 'ห้องน้ำ', box: [27, 35, -24, -16], door: 'n', color: '#d5e0e6', kind: 'toilet' },
  // ฝั่งซ้ายบน + ขวาล่าง
...
function buildWalls() {
  ...
  for (const r of ROOMS) {
    ...
    const eN = Math.abs(z0 + MAP_D / 2) < NEAR_EDGE, eS = Math.abs(z1 - MAP_D / 2) < NEAR_EDGE;
    const eW = Math.abs(x0 + MAP_W / 2) < NEAR_EDGE, eE = Math.abs(x1 - MAP_W / 2) < NEAR_EDGE;
...
function buildProps() {
  ...
  for (const r of ROOMS) {
    ...
    if (r.kind === 'class') {
      ...
      for (let x = -31; x <= 31; x += 6) {
        if (Math.abs(x - (-25)) < 3 || Math.abs(x) < 3) continue;
        add('lockerbay', x, z0 + 1.2, R0);
      }
      for (const x of [-16, 8, 26]) add('waterfountain', x, z0 + 1.2, R0);
      for (let x = -28; x <= 20; x += 9) add('benchlong', x, z1 - 1.8, R180);
      for (const x of [-24, -6, 12, 30]) add('potplant', x, z1 - 3.2, 0);
      ...
      for (const x of [-34, -3, 33]) add('shoerack', x, z1 - 1.5, R180);
