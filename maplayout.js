...
const PROP_SIZE = {
  ...
  cup: [0.57, 0.13],
  icebox: [0.65, 0.48],
  schoolchair: [0.87, 0.30],
  bookshelf: [1.90, 0.53],
  schoolbin: [0.71, 0.30],
  teacherdesk: [0.85, 0.79],
  potplant: [1.19, 0.42],
  watercooler: [1.51, 0.30],
  moppail: [1.33, 0.30],
  lunchtray: [0.14, 0.34],
  standfan: [1.40, 0.32],
  binset: [0.68, 0.66],
  shoerack: [0.80, 0.60],
  noticeboard: [2.10, 0.90],
  schoolbag: [0.65, 0.24],
  waterjar: [0.84, 0.35],
  benchlong: [0.97, 1.15],
  projector: [0.20, 0.26],
  microscope: [0.71, 0.18],
  conesport: [0.53, 0.22],
  clocksign: [0.50, 0.27],
  chalkbox: [0.21, 0.16],
  schooldesk: [0.78, 0.44],
  blackboard: [2.29, 1.40],
  lockerbay: [1.89, 0.94],
  globe: [0.73, 0.19],
  bookstack: [0.31, 0.22],
  basketball: [0.26, 0.15],
  hoop: [3.25, 0.60],
  flagpole: [4.20, 0.60],
  labtable: [1.18, 0.88],
  piano: [1.03, 0.80],
  waterfountain: [1.06, 0.35],
  podium: [1.21, 0.45],
  diningchair: [1.06, 0.31],
  homeshelf: [1.60, 0.48],
  homebin: [0.59, 0.23],
  kitchensink: [1.18, 0.58],
  gardentree: [2.55, 0.90],
  hedge: [0.96, 0.98],
  gardenlamp: [1.49, 0.22],
  toolcart: [0.86, 0.48],
  laundrybasket: [0.71, 0.36],
  coffeetable: [0.46, 0.58],
  bed: [1.05, 0.95],
  wardrobe: [2.10, 0.62],
  toilet: [0.96, 0.36],
  bathtub: [0.93, 0.82],
...
/* ส่งออกตารางขนาดตั้งแต่ตรงนี้ เพราะ ไฟล์ผังด่าน require กลับมาหาไฟล์นี้ระหว่างที่ยังโหลดไม่จบ */
module.exports.PROP_SIZE = PROP_SIZE;
...
function buildNav(layout) {
  ...
  /* เผื่อระยะให้มากกว่าที่ฟิสิกส์ใช้จริง (รัศมีผู้เล่น 0.42 + 0.05)
     ถ้าเผื่อน้อยเกินไป เส้นทางจะเฉียดของจนบอทเดินไปเสียดสีแล้วติด */
  const NODE_PAD = 0.62;
  const EDGE_PAD = 0.58;
  const raw = pts.filter((p) => clearLine(p.x, p.z, p.x, p.z, walls, 0.62)
    && clearOfProps(p.x, p.z, p.x, p.z, grid, NODE_PAD));
  const link = (list) => {
    const e = list.map(() => []);
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (d > 3.6) continue;
        if (!clearLine(list[i].x, list[i].z, list[j].x, list[j].z, walls, 0.55)) continue;
        if (!clearOfProps(list[i].x, list[i].z, list[j].x, list[j].z, grid, EDGE_PAD)) continue;
        e[i].push({ n: j, d });
        e[j].push({ n: i, d });
      }
    }
...
const school = require('./mapschool');
MAPS.school = {
  name: 'โรงเรียน',
  w: school.MAP_W, d: school.MAP_D,
  theme: 'school',
  build() {
    const walls = school.buildWalls();
    const props = school.buildProps();
    const zones = school.ROOMS.map((r) => {
      const [x0, x1, z0, z1] = r.box, cx = (x0 + x1) / 2, cz = (z0 + z1) / 2;
      const doorAt = !r.door ? null
        : r.door === 'n' ? { x: cx, z: z0, half: 1.8 } : r.door === 's' ? { x: cx, z: z1, half: 1.8 }
        : r.door === 'w' ? { x: x0, z: cz, half: 1.8 } : { x: x1, z: cz, half: 1.8 };
      return { name: r.name, box: r.box, door: r.door, doorAt, kind: r.door ? 'building' : 'shop', color: r.color };
    });
    // คนซ่อนเกิดในห้องเรียน/ห้องสมุด คนหาเริ่มที่ลานหน้าเสาธง
    const spawns = {
      hider: [{ x: -27, z: 16 }, { x: -7, z: 16 }, { x: 14, z: 16 }, { x: -26, z: -18 }, { x: -2, z: -18 }, { x: 31, z: -4 }],
      seeker: [{ x: -5, z: -6 }],
    };
    return { walls, props, zones, spawns, nav: buildNav({ walls, props, w: school.MAP_W, d: school.MAP_D, seeds: school.seeds() }) };
  },
};

const house = require('./maphouse');
MAPS.house = {
  name: 'บ้าน',
  w: house.MAP_W, d: house.MAP_D,
  theme: 'house',
  build() {
    const walls = house.buildWalls();
    const props = house.buildProps();
    const zones = house.ROOMS.map((r) => ({ name: r.name, box: r.box, door: r.door, kind: 'room', color: r.color,
      doorAt: (() => { const [x0, x1, z0, z1] = r.box; const cx = (x0 + x1) / 2;
        return r.door === 's' ? { x: cx, z: z1, half: 1.2 } : r.door === 'n' ? { x: cx, z: z0, half: 1.2 } : { x: x1, z: 10, half: 1.8 }; })() }))
      .concat(house.YARDS.map((y) => ({ name: y.name, box: y.box, door: null, kind: 'yard' })));
    // คนซ่อนเกิดในบ้านหลายจุด คนหาเริ่มที่ประตูรั้วหน้าบ้าน
    const spawns = { hider: [{ x: -25, z: -10 }, { x: -5, z: -10 }, { x: -27, z: 12 }, { x: -2, z: 6 }, { x: 30, z: -8 }], seeker: [{ x: 0, z: 27 }] };
    return { walls, props, zones, spawns, nav: buildNav({ walls, props, w: house.MAP_W, d: house.MAP_D, seeds: house.seeds() }) };
  },
};
MAPS.zoo = {
  name: 'สวนสัตว์',
  ...
  build() {
    ...
    const zones = zoo.PENS.map((p) => ({ name: p.name, box: p.box, door: p.gate, kind: p.water ? 'water' : 'pen' }))
      .concat(zoo.BUILDINGS.map((b) => {
        const [x0, x1, z0, z1] = b.box, cx = (x0 + x1) / 2, cz = (z0 + z1) / 2;
        const doorAt = b.door === 'n' ? { x: cx, z: z0, half: 1.8 } : b.door === 's' ? { x: cx, z: z1, half: 1.8 }
          : b.door === 'w' ? { x: x0, z: cz, half: 1.8 } : { x: x1, z: cz, half: 1.8 };
        return { name: b.name, box: b.box, door: b.door, doorAt, kind: 'building' };
      }));
