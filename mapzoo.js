...
/* ของชิ้นเล็กต้องอยู่บนผิวบนของเฟอร์นิเจอร์ ไม่กองบนพื้น */
const SMALL = new Set(['zoocap', 'popcorn', 'plushlion', 'plushpanda', 'keychain', 'slushie',
  'icecream', 'soapbox', 'coffee', 'tray', 'cup', 'balloon']);
...
function buildProps() {
  ...
  const smallSlot = new Map();
  /* วางของชิ้นเล็กบนเฟอร์นิเจอร์ชิ้นที่ใกล้ที่สุดในรัศมี 2 ม. ถ้าไม่มีก็ไม่วาง */
  const addSmall = (t, x, z, ry) => {
    let best = null, bd = 2;
    for (const q of P) {
      if (q.y > 0.05) continue;
      if (!['shelf', 'desk', 'showcase', 'rack', 'fridge', 'sofa', 'chair'].includes(q.t)) continue;
      const d = Math.hypot(q.x - x, q.z - z);
      if (d < bd) { bd = d; best = q; }
    }
    if (!best) return false;
    const kk = best.x + ',' + best.z;
    const u = smallSlot.get(kk) || 0;
    if (u >= 3) return false;
    smallSlot.set(kk, u + 1);
    const oa = [0, -0.6, 0.6][u];
    const ang = best.ry || 0;
    P.push({ t, x: +(best.x + Math.cos(ang) * oa).toFixed(2), z: +(best.z - Math.sin(ang) * oa).toFixed(2),
      ry: ry || ang, y: +(SZ[best.t] ? SZ[best.t][0] : 0.8).toFixed(2) });
    return true;
  };
  const place = (t, x, z, ry) => (SMALL.has(t) ? addSmall(t, x, z, ry) : add(t, x, z, ry));
  ...
  for (const b of BUILDINGS) {
    ...
    if (b.dining) {
      ...
      place('coffee', x0 + 3.4, z0 + 2.2); place('cup', x0 + 5, z0 + 2.2);
      place('tray', x0 + 6.6, z0 + 2.2); add('icebox', x0 + 8.6, z0 + 4.2, R0);
    ...
    for (const side of ['n', 's', 'w', 'e']) {
      ...
      for (let t = s0 + 0.4; t <= s1 - 0.4; t += 1.7) { if (along) place(next(), +t.toFixed(2), inner, face); else place(next(), inner, +t.toFixed(2), face); }
    }
