...
(async function run() {
  ...
  for (const mp of ['school', 'zoo']) {
    const sub = r.body.items.filter((i) => i.map === mp);
    const tot = sub.reduce((a, i) => a + i.chance, 0);
    ok(Math.abs(tot - 100) < 4, `โอกาสออกของ${mp === 'zoo' ? 'สวนสัตว์' : 'โรงเรียน'}รวมกันได้ ${tot.toFixed(1)}% จาก ${sub.length} ชิ้น`);
  }
...
{
  const owned = new Set(store.userByToken(tokA).unlocked);
  const missing = store.poolFor(null).filter((c) => !owned.has(c.id));
  ok(missing.length === 0, `สุ่มไปเรื่อย ๆ ได้ครบทุกชิ้นที่ร้านขาย (${store.poolFor(null).length} ชนิด)`);
}
  ...
  ok(r.body.max === 25, 'ห้องรับคนจริงได้สูงสุด 25 คน (20 ซ่อน : 5 หา)');
