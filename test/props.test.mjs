...
for (const t of PROP_TYPES) {
  ...
  const big = SCENERY.includes(t) || ['giraffe', 'elephant', 'camel', 'deer', 'bamboo', 'flamingo', 'zebra',
    'hoop', 'flagpole', 'skeleton', 'wardrobe'].includes(t);
  const lim = big ? 10 : 3;
...
ok(PROP_TYPES.length <= 190, `ของทั้งเกมใช้ไม่เกิน ${PROP_TYPES.length} draw call (instanced ชนิดละก้อน เฉพาะชนิดที่มีในด่านนั้น)`);
