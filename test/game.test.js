...
/* ---------- ผู้ชมเป็นกล้องบิน เลื่อนดูได้ทั้งแมป ---------- */
{
  ...
  for (const yaw of [0, Math.PI / 2, Math.PI, -Math.PI / 2]) {
    ...
    ok(Math.abs(ps.x) <= rs.map.w / 2 + CFG.SPEC_OUT + 0.01 && Math.abs(ps.z) <= rs.map.d / 2 + CFG.SPEC_OUT + 0.01,
      `เลื่อนไปทางทิศ ${Math.round(yaw * 57.3)} องศาจนสุด เลยขอบได้ไม่เกิน ${CFG.SPEC_OUT} ม.`);
  }
  const reach = Math.max(Math.abs(ps.x), Math.abs(ps.z));
  ok(reach > Math.min(rs.map.w, rs.map.d) / 2 + 5, `เลื่อนเลยขอบแมปออกไปได้จริง ไม่มีกำแพงล่องหน (ไปถึง ${reach.toFixed(0)} ม. จากกลาง)`);
...
/* ---------- เดินไม่กระตุก: เซิร์ฟเวอร์จำลองทีละอินพุต ตำแหน่งตรงกับ ack เสมอ ---------- */
{
  const STEP0 = 1 / CFG.TICK_HZ;
  const rj = new Room('jitter');
  const pj2 = rj.add(fakeSock('jt'), fakeUser('นักเดิน'));
  rj.startRound(); rj.timer = 0; rj.update(0.01);
  pj2.role = 'seeker'; pj2.locked = false; pj2.hold = false;
  let sx = null, sz = null;
  outerJ: for (let x = -rj.map.w / 2 + 4; x < rj.map.w / 2 - 4; x += 1) {
    for (let z = -rj.map.d / 2 + 4; z < rj.map.d / 2 - 20; z += 1) {
      let clear = true;
      for (let t = 0; t <= 16 && clear; t++) {
        if (rj.props.some((q) => !(q.y > 0.05) && Math.hypot(q.x - x, q.z - (z + t)) < 1.6)) clear = false;
        if (rj.walls.some((w) => x + 1 > w.x && x - 1 < w.x + w.w && z + t + 1 > w.z && z + t - 1 < w.z + w.d)) clear = false;
      }
      if (clear) { sx = x; sz = z; break outerJ; }
    }
  }
  ok(sx !== null, `หาทางเดินโล่งยาว 16 ม. สำหรับทดสอบได้ (ด่าน ${rj.mapKey})`);
  if (sx === null) { sx = 0; sz = 0; }
  // ปิดการจบรอบระหว่างทดสอบ จะได้วัดการเดินล้วน ๆ
  rj.timer = 999;
  pj2.x = sx; pj2.z = sz; pj2.y = 0; pj2.vy = 0; pj2.ground = true; pj2.inQ = [];
  const spd = rj.speedOf(pj2);
  let cz = sz, seq = 0, worst = 0;
  const pending = [], inflight = [];
  // เดินให้สั้นกว่าทางโล่งที่หาไว้ (16 ม.) จะได้ไม่ไปชนผนังปลายทาง
  const ticks = Math.floor(14 / (spd * STEP0));
  for (let tick = 0; tick < ticks; tick++) {
    const c = { q: ++seq, f: 1, s: 0, yaw: 0 };
    pending.push(c); inflight.push(c); cz += spd * STEP0;
    const arrive = [0, 1, 1, 2][tick % 4];                  // อินพุตมาถึงไม่สม่ำเสมอเหมือนเน็ตจริง
    for (let k = 0; k < arrive && inflight.length; k++) {
      const it = inflight.shift();
      pj2.inQ.push({ f: it.f, s: it.s, yaw: it.yaw, q: it.q, j: false });
    }
    rj.update(STEP0);
    if (tick % 3 === 2 && tick > 10) {
      while (pending.length && pending[0].q <= pj2.ack) pending.shift();
      const predicted = pj2.z + pending.length * spd * STEP0;
      worst = Math.max(worst, Math.abs(predicted - cz));
      cz = predicted;
    }
  }
  ok(worst < 0.02, `เน็ตแกว่งแล้วตัวละครไม่ถูกดึงถอยหลัง (คลาดสูงสุด ${(worst * 100).toFixed(1)} ซม.)`);
  const sv = require('fs').readFileSync(__dirname + '/../server.js', 'utf8');
  ok(/player\.inQ\.push/.test(sv), 'อินพุตถูกเก็บเป็นคิว ไม่เขียนทับกัน');
  ok(/CFG\.SPD_RUN : 1\)\)/.test(sv), 'ส่งความเร็วรวมตัวคูณวิ่งให้ไคลเอนต์ทำนายตรงตอนวิ่ง');
}

/* ---------- ต่อกลับแล้วไม่ค้างเป็นภาพมุมสูง ---------- */
{
  const rp0 = new Room('initpay');
  const pp0 = rp0.add(fakeSock('ip'), fakeUser('ต่อกลับ'));
  const srv = require('../server.js');
  const sv = require('fs').readFileSync(__dirname + '/../server.js', 'utf8');
  ok((sv.match(/socket\.emit\('init', initPayload\(room, player\)\)/g) || []).length === 2,
    'เข้าครั้งแรกกับต่อกลับใช้ข้อมูลตั้งต้นชุดเดียวกัน');
  ok(/types: PROP_TYPES/.test(sv.slice(sv.indexOf('function initPayload'))) && /tickHz/.test(sv.slice(sv.indexOf('function initPayload'))),
    'ข้อมูลตั้งต้นมีตารางชนิดของและอัตราติ๊กครบ (เดิมตอนต่อกลับขาด จอเลยค้าง)');
}

/* ---------- กล้องผู้ชมไม่หลุดขอบแมป ทิศซ้ายขวาตรงกับการเดินปกติ ---------- */
{
  const g = require('fs').readFileSync(__dirname + '/../public/game.js', 'utf8');
  ok(/ghostCam && MAP && MAP\.w/.test(g), 'ตำแหน่งกล้องผู้ชมถูกล็อกไว้ในขอบแมป');
  const rs5 = new Room('specdir');
  const ps5 = rs5.add(fakeSock('sd'), fakeUser('ดูทิศ'));
  rs5.startRound(); rs5.timer = 0; rs5.update(0.01);
  // เดินปกติ: กด s=1 ไปทางไหน
  const pw = rs5.list().find((q) => q !== ps5) || ps5;
  const yaw = 0.7;
  const walkDx = Math.sin(yaw) * 0 - Math.cos(yaw) * 1, walkDz = Math.cos(yaw) * 0 + Math.sin(yaw) * 1;
  ps5.role = 'out'; ps5.x = 0; ps5.z = 0; ps5.in = { f: 0, s: 1, yaw, seq: 1 };
  rs5.updateSpectators(1 / 30);
  const dot = ps5.x * walkDx + ps5.z * walkDz;
  ok(dot > 0, 'กล้องผู้ชมเลื่อนซ้าย-ขวาทิศเดียวกับตอนเดินปกติ');
}


/* ---------- ทุกห้องมีทางเข้าออกทางเดียวคือประตู ไม่มีทางหนีออกหลังห้อง ---------- */
{
  for (const key of Object.keys(LAYOUTS)) {
    const L = LAYOUTS[key];
    const walls = L.walls.filter((w) => w.h > 1.5);           // รั้วเตี้ยกระโดดข้ามได้ ไม่นับ
    const R = CFG.R;
    const hit = (x, z) => walls.some((w) => x + R > w.x && x - R < w.x + w.w && z + R > w.z && z - R < w.z + w.d);
    const rooms = (L.zones || []).filter((z) => z.door && z.doorAt);
    const leaks = [];
    for (const zn of rooms) {
      const [x0, x1, z0, z1] = zn.box, d = zn.doorAt, half = (d.half || 1.8) + 0.6;
      // อุดประตูไว้ แล้วดูว่ายังเดินออกจากห้องทางอื่นได้ไหม
      const sealed = (x, z) => (zn.door === 'n' || zn.door === 's')
        ? Math.abs(z - d.z) < 1.2 && Math.abs(x - d.x) < half
        : Math.abs(x - d.x) < 1.2 && Math.abs(z - d.z) < half;
      const step = 0.5, k2 = (x, z) => Math.round(x / step) + ',' + Math.round(z / step);
      const cx = (x0 + x1) / 2, cz = (z0 + z1) / 2;
      let st = null;
      for (let r2 = 0; r2 < 6 && !st; r2 += 0.5) {
        for (let a = 0; a < 12 && !st; a++) {
          const x = cx + Math.cos(a) * r2, z = cz + Math.sin(a) * r2;
          if (!hit(x, z) && !sealed(x, z)) st = [x, z];
        }
      }
      if (!st) continue;
      const seen = new Set([k2(...st)]), q = [st];
      let escaped = false;
      while (q.length && !escaped) {
        const [x, z] = q.pop();
        for (const [dx, dz] of [[step, 0], [-step, 0], [0, step], [0, -step]]) {
          const nx = x + dx, nz = z + dz, kk = k2(nx, nz);
          if (seen.has(kk)) continue;
          seen.add(kk);
          if (Math.abs(nx) > L.w / 2 || Math.abs(nz) > L.d / 2) continue;
          if (hit(nx, nz) || sealed(nx, nz)) continue;
          if (nx < x0 - 1.2 || nx > x1 + 1.2 || nz < z0 - 1.2 || nz > z1 + 1.2) { escaped = true; break; }
          q.push([nx, nz]);
        }
      }
      if (escaped) leaks.push(zn.name);
    }
    ok(rooms.length > 0 && leaks.length === 0,
      `[${L.name}] ${rooms.length} ห้องมีผนังล้อมรอบ เข้าออกได้ทางประตูเท่านั้น${leaks.length ? ' — รั่ว: ' + leaks.join(',') : ''}`);
  }
}

/* ---------- กล้องผู้ชมไม่หลุดขอบแมป (ทุกชั้นของการคำนวณ) ---------- */
{
  const g = require('fs').readFileSync(__dirname + '/../public/game.js', 'utf8');
  ok(/ghostCam && MAP && MAP\.w/.test(g), 'ตำแหน่งกล้องมีขอบเขต');
  ok(/const mx = MAP\.w \/ 2 \+ CAM_OUT/.test(g), 'จุดที่กล้องมองมีขอบเขต (เลยขอบแมปได้ CAM_OUT เมตร ไม่ไปไกลไม่สิ้นสุด)');
  const sim = g.slice(g.indexOf('function simulate'), g.indexOf('function simulate') + 900);
  ok(/Math\.min\(mw \/ 2 \+ CAM_OUT/.test(sim), 'ตัวทำนายตำแหน่งผู้ชมใช้ขอบเขตเดียวกับเซิร์ฟเวอร์');
}

/* ---------- เพลงเปิดเองตอนเข้าเกม ---------- */
{
  const app = require('fs').readFileSync(__dirname + '/../public/app.js', 'utf8');
  const mu = require('fs').readFileSync(__dirname + '/../public/music.js', 'utf8');
  ok(/export function unlock/.test(mu), 'มีฟังก์ชันปลุกเสียงจากการแตะจอ');
  const play = app.slice(app.indexOf("$('btnPlay').onclick"), app.indexOf("$('btnPlay').onclick") + 300);
  ok(/music\.unlock\(true\)/.test(play), 'กดเล่นเลยแล้วเปิดเพลงในจังหวะที่กดพอดี (เบราว์เซอร์ยอมให้มีเสียง)');
  ok(/'touchend', 'click'/.test(app), 'ฟังการแตะแบบ touchend/click ด้วย (iOS ปลดล็อกเสียงจาก pointerdown อย่างเดียวไม่ได้)');
  ok(!/removeEventListener\('pointerdown', autoMusic\)/.test(app), 'ไม่ถอดตัวฟังหลังครั้งแรก เผื่อระบบพักเสียงตอนสลับแอป');
  ok(/visibilitychange/.test(app), 'กลับเข้าแอปแล้วลองปลุกเสียงอัตโนมัติ');
}


/* ---------- กล้องส่องตอนล็อกท่า: เลื่อนได้ทั้งแมป ทะลุผนัง ไม่มีกำแพงล่องหน ---------- */
{
  const g = require('fs').readFileSync(__dirname + '/../public/game.js', 'utf8');
  ok(!/const lim = 34/.test(g), 'เลิกจำกัดกล้องล็อกท่าไว้แค่ 34 ม. จากตัวแล้ว');
  ok(/MAP\.w \/ 2 \+ CAM_OUT/.test(g), 'กล้องล็อกท่าเลื่อนได้ทั้งแมปและเลยขอบออกไปได้');
  ok(/\|\| freeMode;/.test(g), 'กล้องล็อกท่าไม่ถูกระบบหลบกำแพงกระชากกลับ (ทะลุผนังได้)');
  ok(/const sp = SPEC_SPEED \* dt;/.test(g), 'กล้องล็อกท่าเลื่อนเร็วเท่ากล้องผู้ชม');
  const cOut = +(g.match(/const CAM_OUT = (\d+)/) || [])[1];
  ok(cOut === CFG.SPEC_OUT, `ขอบเขตกล้องฝั่งไคลเอนต์ (${cOut}) ตรงกับเซิร์ฟเวอร์ (${CFG.SPEC_OUT}) ไม่งั้นจะถูกดึงกลับ`);
}


/* ---------- ชุดนักล่าปีศาจ (ออกแบบเอง ไม่อิงตัวละครลิขสิทธิ์) ---------- */
{
  const src = require('fs').readFileSync(__dirname + '/../public/props.js', 'utf8');
  const skinBlock = src.slice(src.indexOf('export const SKINS'), src.indexOf('export const SKIN_IDS'));
  const ids = [...skinBlock.matchAll(/^  ([A-Za-z]+):/gm)].map((m) => m[1]);
  const slayers = ids.filter((i) => /^slayer/.test(i));
  ok(slayers.length === 6, `มีชุดนักล่าปีศาจ ${slayers.length} แบบ`);

  // ต้องมีทั้งชายและหญิง
  const male = (skinBlock.match(/gender: 'm'/g) || []).length;
  const female = (skinBlock.match(/gender: 'f'/g) || []).length;
  ok(male >= 3 && female >= 3, `มีทั้งชาย ${male} แบบ และหญิง ${female} แบบ`);

  // ทุกแบบต้องมีฮาโอริ ลาย ทรงผม และดาบ
  for (const id of slayers) {
    const line = skinBlock.slice(skinBlock.indexOf('  ' + id + ':'));
    const one = line.slice(0, line.indexOf('},') + 2);
    ok(/haori:/.test(one) && /pattern:/.test(one) && /katana:/.test(one) && /cap: '/.test(one),
      `[${id}] มีครบทั้งฮาโอริ ลาย ทรงผม และดาบ`);
  }

  // ร้านค้าต้องขายครบและไม่มีสกินไหนขาดโมเดล
  const catIds = store.SKIN_CATALOG.map((c) => c.id);
  ok(catIds.length === ids.length && catIds.every((c) => ids.includes(c)),
    `ร้านขายสกินครบ ${catIds.length} แบบ ตรงกับโมเดลทุกตัว`);

  // ป้องกันการเผลอใส่ชื่อตัวละครจากเรื่องที่มีลิขสิทธิ์
  const banned = ['tanjiro', 'nezuko', 'zenitsu', 'inosuke', 'giyu', 'rengoku', 'kimetsu',
    'demonslayer', 'ทันจิโร่', 'เนซึโกะ', 'เซนอิตสึ', 'อิโนสึเกะ', 'ดาบพิฆาตอสูร'];
  const low = src.toLowerCase();
  const found = banned.filter((b) => low.includes(b.toLowerCase()));
  ok(found.length === 0, `ไม่มีชื่อตัวละคร/ชื่อเรื่องที่มีลิขสิทธิ์อยู่ในโค้ด${found.length ? ' — เจอ ' + found.join(',') : ''}`);
}


/* ---------- ปุ่มแถบล่างทุกปุ่มต้องเปิดแผงได้จริง ---------- */
{
  const html = require('fs').readFileSync(__dirname + '/../public/index.html', 'utf8');
  const app = require('fs').readFileSync(__dirname + '/../public/app.js', 'utf8');
  const btnPanels = [...html.matchAll(/data-panel="([a-zA-Z]+)"/g)].map((m) => m[1]);
  const listed = (app.match(/const PANELS = \[([^\]]+)\]/) || [])[1] || '';
  const miss = [...new Set(btnPanels)].filter((p2) => !listed.includes(`'${p2}'`));
  ok(miss.length === 0,
    `ทุกปุ่มเมนูมีชื่อแผงอยู่ในรายการ PANELS แล้ว${miss.length ? ' — ขาด ' + miss.join(',') : ''}`);
  for (const p2 of new Set(btnPanels)) {
    ok(html.includes(`id="${p2}"`), `มีแผง ${p2} อยู่ใน HTML จริง`);
  }
  ok(/curPanel === 'pFriends'\) loadFriends\(\)/.test(app), 'เปิดแผงเพื่อนแล้วโหลดรายชื่อให้ด้วย');
}
...
{
  const g = require('fs').readFileSync(__dirname + '/../public/game.js', 'utf8');
  ok(/roofs\.push/.test(g), 'ห้องที่มีผนังล้อมรอบมีหลังคาคลุม');
  ok(/roofMesh\.visible = !inside/.test(g), 'เข้าไปในห้องแล้วหลังคาซ่อนให้อัตโนมัติ');
  ok(/zn\.kind !== 'pen' && zn\.kind !== 'water' && zn\.kind !== 'yard'/.test(g),
    'คอกสัตว์กับสนามกลางแจ้งไม่มีหลังคา');
  // นับว่าแต่ละด่านจะได้หลังคากี่หลัง
  for (const key of Object.keys(LAYOUTS)) {
    const roofed = (LAYOUTS[key].zones || []).filter((z) => z.door
      && z.kind !== 'pen' && z.kind !== 'water' && z.kind !== 'yard').length;
    ok(roofed > 0, `[${LAYOUTS[key].name}] มีอาคารที่ได้หลังคา ${roofed} หลัง`);
  }
}

/* ---------- กล้องก้มเงยได้กว้างพอ ---------- */
{
  const g = require('fs').readFileSync(__dirname + '/../public/game.js', 'utf8');
  const mn = parseFloat((g.match(/PITCH_MIN = (-?[0-9.]+)/) || [])[1]);
  const mx = parseFloat((g.match(/PITCH_MAX = (-?[0-9.]+)/) || [])[1]);
  ok(mx - mn > 1.5, `ช่วงก้มเงยกว้าง ${Math.round((mx - mn) * 57.3)} องศา (${Math.round(mn * 57.3)} ถึง ${Math.round(mx * 57.3)})`);
  ok((g.match(/PITCH_MIN, Math\.min\(PITCH_MAX/g) || []).length >= 2,
    'ทั้งลากนิ้วบนมือถือและลากเมาส์ใช้ช่วงเดียวกัน');
}

/* ---------- แบ่งบทบาทต้องเป็นธรรม ไม่ใช่คนเดิมเป็นคนหาทุกรอบ ---------- */
{
  const rf = new Room('fairness');
  const players = [];
  for (let i = 0; i < 5; i++) players.push(rf.add(fakeSock('f' + i), fakeUser('เพื่อน' + i)));
  const seekCount = {}, streak = {};
  let worstStreak = 0;
  for (let round = 0; round < 120; round++) {
    rf.startRound();
    for (const p of players) {
      if (p.role === 'seeker') {
        seekCount[p.name] = (seekCount[p.name] || 0) + 1;
        streak[p.name] = (streak[p.name] || 0) + 1;
        worstStreak = Math.max(worstStreak, streak[p.name]);
      } else streak[p.name] = 0;
    }
  }
  const counts = players.map((p) => seekCount[p.name] || 0);
  const lo = Math.min(...counts), hi = Math.max(...counts);
  ok(lo > 0, `ทุกคนได้เป็นคนหาบ้าง (น้อยสุด ${lo} ครั้งจาก 120 รอบ)`);
  ok(hi - lo <= 25, `โอกาสเป็นคนหาใกล้เคียงกัน (${lo}-${hi} ครั้ง)`);
  ok(worstStreak <= 3, `ไม่มีใครเป็นคนหาติดกันยาว (นานสุด ${worstStreak} รอบ)`);
  const sv = require('fs').readFileSync(__dirname + '/../server.js', 'utf8');
  // ตรวจเฉพาะโค้ดจริง ไม่นับที่อยู่ในคอมเมนต์อธิบายบั๊กเดิม
  const codeOnly = sv.split('\n').filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l)).join('\n');
  ok(!/sort\(\(\) => Math\.random\(\) - 0\.5\)/.test(codeOnly),
    'เลิกใช้วิธีสลับที่ลำเอียง (sort สุ่ม) แล้ว');
  ok(/\[order\[i\], order\[j\]\] = \[order\[j\], order\[i\]\]/.test(sv), 'ใช้การสลับแบบ Fisher-Yates');
}
